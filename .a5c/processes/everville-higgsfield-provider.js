  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Define stable provider, operation, policy, request, result, cancellation, and idempotency contracts without selecting local-versus-cloud topology.',
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Cover typed allowlisting, argv-array construction through an injected executor, no auth token or arbitrary pass-through, no shell interpolation, browser unsupported behavior, stable idempotency metadata, cancellation, and fail-closed error normalization.',
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Use an injected executor accepting an executable plus argv array and AbortSignal. Never use a shell, concatenate a command string, invoke auth token, or provide arbitrary command pass-through.',
        ...
        'Normalize all expected failures into stable fail-closed codes without raw stdout, stderr, credentials, paths, URLs with credentials, or Error causes.',
...
const focusedGateTask = defineTask('run-higgsfield-provider-focused-gates', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'pnpm run typecheck',
      `if rg -n "child_process\\.(exec|execSync)|shell\\s*:\\s*true|auth[ ._-]*token|arbitrary.{0,20}(command|argv)" ${implementationPaths.map(quote).join(' ')}; then echo 'Unsafe CLI execution primitive found'; exit 1; fi`,
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Fail on scope reduction, changed frozen tests, incomplete CLI inventory, unstable IDs, product-gated execution, auth token exposure, arbitrary pass-through, shell interpolation, missing cancellation/idempotency, secret/path/raw-error leakage, browser import hazards, Electron behavior drift, or cloud-vendor coupling.',
