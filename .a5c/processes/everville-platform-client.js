...
const preflightTask = defineTask('platform-client-preflight', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'for file in src/platform/client.ts src/platform/electronPlatformClient.ts src/platform/browserPlatformClient.ts src/platform/platformClient.contract.test.ts src/platform/platformClient.boundary.test.ts docs/architecture/platform-client-runtime-boundary.md; do if [ -e "$file" ]; then echo "EXISTS $file"; else echo "NEW $file"; fi; done',
      "printf '%s\\n' '--- LIVE BRIDGE REFERENCES ---'",
      "rg -n 'getDesktopBridge|window\\.nomiDesktop' src electron --glob '*.{ts,tsx}'",
      "printf '%s\\n' '--- SELECTED CALL PATHS ---'",
      `for file in ${selectedPaths.map(quote).join(' ')}; do echo "--- $file ---"; sed -n '1,260p' "$file"; done`,
      "printf '%s\\n' '--- BRIDGE TYPES ---'",
