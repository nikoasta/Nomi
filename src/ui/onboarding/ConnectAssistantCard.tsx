...
export function ConnectAssistantCard({ info, onChanged }: ConnectAssistantCardProps): JSX.Element | null {
  ...
  const handleInstall = () => {
    ...
    try {
      ...
      onChanged()
      toast(tt('assistant.installToast', { label }), 'success')
    } catch (e) {
      setError(tt('assistant.installError', { message: e instanceof Error ? e.message : String(e) }))
    } finally {
      setBusy(false)
    }
  }
  ...
  const handleUninstall = () => {
    ...
    try {
      ...
      onChanged()
      toast(tt('assistant.uninstallToast'), 'success')
    } catch (e) {
      setError(tt('assistant.uninstallError', { message: e instanceof Error ? e.message : String(e) }))
    } finally {
      setBusy(false)
    }
  }
  ...
  const handleCopy = () => {
    void navigator.clipboard.writeText(client.snippet).then(() => {
      setCopied(true)
      toast(tt('assistant.copyToast'), 'success')
      window.setTimeout(() => setCopied(false), 1600)
    })
  }

  const statusLabel = client.installed ? tt('assistant.installedStatus') : info.tokenReady ? tt('assistant.readyStatus') : tt('assistant.notReadyStatus')

  return (
    <FoldableModelCard
      glyph={<IconTerminal2 size={16} stroke={1.6} />}
      glyphTone="ink"
      name={tt('assistant.name')}
      subtitle={tt('assistant.subtitle')}
            ...
            <>
              <div className="flex items-start gap-2 rounded-nomi-sm bg-[var(--workbench-success-soft)] px-3 py-2.5">
                ...
                <div className="min-w-0">
                  <div className="text-body-sm font-semibold text-nomi-ink">{tt('assistant.configWritten', { label })}</div>
                  <div className="text-caption text-nomi-ink-60 mt-0.5">{tt('assistant.restart', { label })}</div>
                </div>
              </div>
              <div className="text-caption text-nomi-ink-40">{tt('assistant.sayThis')}</div>
              ...
              >
                {tt('assistant.uninstall')}
              </button>
            </>
          ) : (
            <>
              <div className="text-caption text-nomi-ink-60 leading-relaxed">
                {tt('assistant.description', { label })}
              </div>
              <button
                type="button"
              ...
              >
                <IconPlugConnected size={15} stroke={1.8} />{tt('assistant.install', { label })}
              </button>
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {copied ? tt('assistant.copied') : tt('assistant.copyConfig')}
                </button>
                <button
                  type="button"
                ...
                >
                  {tt('assistant.guide')}<IconExternalLink size={13} stroke={1.6} />
                </button>
              </div>
              <div className="text-micro text-nomi-ink-30">{tt('assistant.otherTools')}</div>
            </>
          )}
        </>
      )}
