...
export function DreaminaMemberCard({ status, onChanged }: DreaminaMemberCardProps): JSX.Element | null {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      if (r.ok) { toast(tt('dreamina.installToast'), 'success'); onChanged() }
  ...
  const runPollLoop = async (deviceCode: string) => {
    ...
    try {
      ...
      for (let i = 0; i < 8 && !cancelPoll.current; i += 1) {
        ...
        if (r.status === 'success') { toast(tt('dreamina.loginToast'), 'success'); setFlow(null); onChanged(); return }
        if (r.status === 'error') { setError(r.message); return }
        // pending → 继续下一轮
      }
      if (!cancelPoll.current) setError(tt('dreamina.timeout'))
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (/复用.*登录态|已登录|already logged in|already authenticated/i.test(message)) {
        toast(tt('dreamina.alreadyLoggedIn'), 'success')
        onChanged()
      } else {
        setError(message)
      }
  ...
  const handleLogout = async () => {
    ...
    setBusy(true); setError('')
    try { await dreamina.logout(); setFlow(null); onChanged(); toast(tt('dreamina.logoutToast'), 'success') }
    catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(false) }
  }
  ...
  const handleCopyLink = () => {
    ...
    void navigator.clipboard.writeText(flow.verificationUri).then(() => {
      setCopied(true); toast(tt('dreamina.copyToast'), 'success'); window.setTimeout(() => setCopied(false), 1600)
    })
  }

  const cardStatus: 'ok' | 'todo' = status.installed && status.loggedIn ? 'ok' : 'todo'
  const statusLabel = !status.installed ? tt('dreamina.notInstalled') : !status.loggedIn ? tt('dreamina.loginPending') : status.notMaestroVip ? tt('dreamina.notVip') : tt('dreamina.loggedIn')

  return (
    <FoldableModelCard
      glyph={<IconMovie size={16} stroke={1.6} />}
      glyphTone="ink"
      name={tt('dreamina.name')}
      subtitle={tt('dreamina.subtitle')}
      status={cardStatus}
      ...
      {!status.installed ? (
        <>
          <div className="text-caption text-nomi-ink-60 leading-relaxed">
            {tt('dreamina.installDescription')}
          </div>
          <button
          ...
          >
            <IconDownload size={15} stroke={1.8} />{busy ? tt('dreamina.installing') : tt('dreamina.installCli')}
          </button>
          <div className="text-micro text-nomi-ink-30">{tt('dreamina.source')}</div>
        </>
      ) : flow ? (
        /* 设备码登录中：显链接 + 验证码 */
        <>
          <div className="flex items-start gap-2 rounded-nomi-sm border border-nomi-line bg-nomi-paper px-3 py-2.5">
            ...
            <div className="min-w-0">
              <div className="text-body-sm font-semibold text-nomi-ink">{tt('dreamina.authorizeTitle')}</div>
              <div className="text-caption text-nomi-ink-60 mt-0.5">{tt('dreamina.authorizeBody')}</div>
            </div>
          </div>
          <button
          ...
          >
            {tt('dreamina.openAuth')}<IconExternalLink size={14} stroke={1.6} />
          </button>
          <div className="flex items-center gap-2">
            <div className="flex-1 text-caption text-nomi-ink-60">{tt('dreamina.code')} <code className="text-nomi-ink font-mono">{flow.userCode.slice(0, 12)}...</code></div>
            <button type="button" onClick={handleCopyLink} className="h-8 px-2 text-caption text-nomi-ink-60 inline-flex items-center gap-1 hover:text-nomi-accent">
              {copied ? <IconCheck size={13} stroke={1.8} /> : <IconCopy size={13} stroke={1.6} />}{copied ? tt('assistant.copied') : tt('dreamina.copyLink')}
            </button>
          </div>
          <div className="text-caption text-nomi-ink-40">{polling ? tt('dreamina.waiting') : tt('dreamina.afterAuth')}</div>
          <button type="button" onClick={() => { cancelPoll.current = true; setFlow(null) }} className="self-start text-caption text-nomi-ink-40 hover:text-workbench-danger">{tt('dreamina.cancel')}</button>
        </>
      ) : status.loggedIn ? (
          ...
          <div className="flex items-start gap-2 rounded-nomi-sm bg-[var(--workbench-success-soft)] px-3 py-2.5">
            ...
            <div className="min-w-0">
              <div className="text-body-sm font-semibold text-nomi-ink">
                {tt('dreamina.loggedInTitle')}
                {status.totalCredit != null ? <span className="text-nomi-ink-60 font-normal"> · {tt('dreamina.credits', { count: status.totalCredit })}</span> : null}
              </div>
              <div className="text-caption text-nomi-ink-60 mt-0.5">
                {tt('dreamina.loggedInBody')}
              </div>
            </div>
          </div>
          <button type="button" onClick={handleLogout} disabled={busy} className="self-start text-caption text-nomi-ink-40 hover:text-workbench-danger disabled:opacity-50">{tt('dreamina.logout')}</button>
        </>
      ) : (
        /* 已装未登录 */
        <>
          <div className="text-caption text-nomi-ink-60 leading-relaxed">
            {tt('dreamina.loginDescription')}
          </div>
          <button
            ...
            className={cn('w-full h-9 rounded-nomi-sm bg-nomi-ink text-nomi-paper text-body-sm font-semibold',
              'inline-flex items-center justify-center gap-1.5 hover:bg-nomi-accent disabled:opacity-50')}
          >
            <IconQrcode size={15} stroke={1.8} />{busy ? tt('dreamina.starting') : tt('dreamina.scanLogin')}
          </button>
        </>
      )}
