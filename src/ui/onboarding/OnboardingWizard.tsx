...
export function OnboardingWizard({ opened, onClose, onCommitted, initialPreset }: {
  ...
  const kindOptions = React.useMemo<Array<{ value: ModelKind; label: string }>>(() => [
    { value: 'image', label: tt('wizard.kind.image') },
    { value: 'video', label: tt('wizard.kind.video') },
    { value: 'audio', label: tt('wizard.kind.audio') },
    { value: 'text', label: tt('wizard.kind.text') },
  ], [tt])
  ...
  const handleFetchModels = React.useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && res.models && res.models.length > 0) {
        ...
        setCandidateModels([])
        setFetchModelsMsg(tt('wizard.fetch.emptyWithManual'))
      } else {
        setCandidateModels([])
        setFetchModelsMsg(tt('wizard.fetch.emptyRetry'))
      }
    } finally {
      setFetchAttempted(true)
      setFetchingModels(false)
    }
  }, [bridge, baseUrl, userApiKey, providerKind, buildHeadersObject, tt])
  ...
  const handleTestConnection = React.useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setTestState('ok')
      setTestMessage(res.detectedKind ? tt('wizard.test.connectedWithProtocol', { protocol: PROVIDER_KIND_LABEL[res.detectedKind] }) : tt('wizard.test.connected'))
    } else {
      setTestState('fail')
      ...
      setTestMessage(res.error
        ? tt('wizard.test.failedWithError', { message: res.error })
        : tt('wizard.test.failed'))
    }
  }, [bridge, baseUrl, userApiKey, models, providerKind, kindForced, buildHeadersObject, tt])

  const handleManualSave = React.useCallback(async () => {
    if (!bridge?.onboarding?.manualCommit) {
      setErrorReason(tt('wizard.error.desktopMissing'))
      setPhase('error')
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResultLabel(n === 1 ? (res.committed?.[0]?.displayName || cleanModels[0].id) : tt('wizard.result.models', { count: n }))
        setPhase('success')
        if (res.committed) onCommitted?.(res.committed)
      } else {
        setErrorReason(tt('wizard.error.saveFailed'))
        setErrorHint(res.error || tt('wizard.error.checkEndpoint'))
        setPhase('error')
      }
    } finally {
      setSaving(false)
    }
  }, [bridge, vendorName, baseUrl, userApiKey, models, providerKind, buildHeadersObject, onCommitted, tt])
      ...
      <Stack gap="md">
        {phase === 'input' && screen === 'form' && (
          <Stack gap={12}>
            ...
            {([
              { key: 'relay', label: tt('wizard.group.relay') },
              { key: 'official', label: tt('wizard.group.official') },
            ] as const).map(grp => {
              const items = PROVIDER_PRESETS.filter(p => (p.group ?? 'official') === grp.key)
              if (items.length === 0) return null
              return (
                <Field key={grp.key} label={grp.label} hint={grp.key === 'relay' ? tt('wizard.group.relayHint') : undefined}>
            ...
            <Field label={tt('wizard.field.sourceName')} hint={tt('wizard.field.sourceNameHint')}>
              <DesignTextInput
                value={vendorName}
                onChange={e => setVendorName(e.currentTarget.value)}
                placeholder={tt('wizard.placeholder.sourceName')}
              />
            </Field>
            {showBaseUrlField ? (
              <Field
                label={tt('wizard.field.baseUrl')}
                hint={providerKind === 'anthropic' ? tt('wizard.field.baseUrlHint.anthropic') : tt('wizard.field.baseUrlHint.relay')}
              >
                <DesignTextInput
                  value={baseUrl}
                  onChange={e => {
                  ...
                  placeholder={providerKind === 'anthropic' ? tt('wizard.placeholder.anthropicBaseUrl') : 'https://api.openai.com/v1'}
                  error={baseUrlTrimmed.length > 0 && !baseUrlValid ? tt('wizard.baseUrl.invalid') : undefined}
                  onBlur={maybeAutoFetchModels}
                />
              </Field>
            ) : (
              <Text size="xs" c="var(--nomi-ink-60)">
                {tt('wizard.baseUrl.prefilled')}{' '}
                <Anchor component="button" type="button" onClick={() => setEditBaseUrl(true)} c="var(--nomi-accent)" inherit>
                  {tt('wizard.baseUrl.customize')}
                </Anchor>
              </Text>
            )}
            <Field label={tt('wizard.field.apiKey')} hint={tt('wizard.field.apiKeyHint')}>
              <PasswordInput
                value={userApiKey}
              ...
              {selectedPreset?.keyUrl && (
                <Anchor href={selectedPreset.keyUrl} target="_blank" rel="noreferrer" c="var(--nomi-accent)" size="xs">
                  {tt('wizard.keyHelp', { label: selectedPreset.label })}
                </Anchor>
              )}
            </Field>
            ...
            <Stack gap={6}>
              <Group gap={6} align="center" wrap="nowrap">
                <Text size="sm" c="var(--nomi-ink)">{tt('wizard.models')}</Text>
                {models.length > 0 && (
                  <Group gap={3} align="center" wrap="nowrap">
                    <IconCheck size={13} stroke={1.5} style={{ color: 'var(--workbench-success)' }} />
                    <Text size="xs" c="var(--workbench-success)">{tt('wizard.models.selected', { count: models.length })}</Text>
                  </Group>
                )}
              </Group>
              ...
              {fetchingModels && candidateModels.length === 0 ? (
                ...
                <div className="flex items-center gap-2.5 rounded-nomi border border-nomi-line px-3.5 py-3">
                  <Loader size="xs" />
                  <Text size="sm" c="var(--nomi-ink-60)">{tt('wizard.models.fetching')}</Text>
                </div>
              ) : models.length > 0 ? (
                ...
                <>
                      ...
                      <Group key={m.id} gap={8} wrap="nowrap" align="center" justify="space-between">
                        ...
                        <Group gap={4} wrap="nowrap" align="center" style={{ flexShrink: 0 }}>
                          <Select
                            ...
                            style={{ width: 88 }}
                          />
                          <ActionIcon variant="subtle" color="gray" onClick={() => removeModel(m.id)} aria-label={tt('wizard.removeModelAria', { name: m.id })}>
                            <IconX size={14} />
                          </ActionIcon>
                        </Group>
                      </Group>
                  ...
                  <Anchor component="button" type="button" size="xs" c="var(--nomi-accent)" onClick={() => setScreen('select')} style={{ alignSelf: 'flex-start' }}>
                    {tt('wizard.models.modifySelection')}
                  </Anchor>
                </>
                ...
                <div className="flex items-center gap-2.5 rounded-nomi border border-nomi-line px-3.5 py-3">
                  ...
                  <Text size="sm" c="var(--nomi-ink-60)" style={{ flex: 1 }}>{tt('wizard.models.candidates', { count: candidateModels.length })}</Text>
                  <DesignButton variant="light" onClick={() => setScreen('select')}>{tt('wizard.models.select')}</DesignButton>
                </div>
              ) : fetchAttempted ? (
                // 拉了但端点没列出 → 去第二屏手填 id（保留逃生口）。
                <div className="flex items-center gap-2.5 rounded-nomi border border-nomi-line px-3.5 py-3">
                  <IconAlertTriangle size={16} stroke={1.5} style={{ color: 'var(--nomi-ink-60)', flexShrink: 0 }} />
                  <Text size="xs" c="var(--nomi-ink-60)" style={{ flex: 1 }}>{fetchModelsMsg || tt('wizard.models.empty')}</Text>
                  <DesignButton variant="light" onClick={() => setScreen('select')}>{tt('wizard.models.manualSelect')}</DesignButton>
                </div>
              ) : (
                // 还没拉 → 提示 + 显式拉取（失焦也会自动拉）。
                <div className="flex items-center gap-2.5 rounded-nomi border border-nomi-line px-3.5 py-3">
                  <IconCloudDownload size={18} stroke={1.6} style={{ color: 'var(--nomi-ink-40)', flexShrink: 0 }} />
                  <Text size="sm" c="var(--nomi-ink-60)" style={{ flex: 1 }}>{tt('wizard.models.readyToFetch')}</Text>
                  <DesignButton variant="light" onClick={handleFetchModels} disabled={!canTest} loading={fetchingModels}>{tt('wizard.models.fetch')}</DesignButton>
                </div>
              )}
            </Stack>
            ...
            <Stack gap={6}>
              ...
              <Collapse in={showAdvanced}>
                <Stack gap={12}>
                  ...
                  {!showKindOverride ? (
                    <Text size="xs" c="var(--nomi-ink-60)">
                      {tt('wizard.protocol.status', { value: kindForced ? PROVIDER_KIND_LABEL[providerKind] : tt('wizard.protocol.autoDetect') })}{' '}
                      <Anchor component="button" type="button" onClick={() => setShowKindOverride(true)} c="var(--nomi-accent)" inherit>
                        {tt('wizard.protocol.manual')}
                      </Anchor>
                    </Text>
                  ) : (
                    <Field label={tt('wizard.protocol.label')} hint={tt('wizard.protocol.hint')}>
                  ...
                  {/* 自定义请求头 */}
                  <Stack gap={4}>
                    {headerRows.length > 0 && <Text size="sm" c="var(--nomi-ink)">{tt('wizard.headers.title')}</Text>}
                    {headerRows.length > 0 && (
                      <Stack gap={6}>
                        {headerRows.map((h, i) => (
                          <Group key={i} gap={6} wrap="nowrap" align="flex-start">
                            <DesignTextInput
                              ...
                              placeholder={tt('wizard.headers.namePlaceholder')}
                              style={{ flex: 1 }}
                            />
                            <DesignTextInput
                              value={h.value}
                              onChange={e => updateHeader(i, { value: e.currentTarget.value })}
                              placeholder={tt('wizard.headers.valuePlaceholder')}
                              style={{ flex: 1 }}
                            />
                            <ActionIcon
                              variant="subtle"
                              ...
                              aria-label={tt('wizard.headers.deleteAria')}
                            >
                              <IconTrash size={14} />
                            </ActionIcon>
                          </Group>
              ...
              <DesignButton
                ...
                title={
                  manualSaveAction === 'arm'
                    ? tt('wizard.save.armTitle')
                    : manualSaveAction === 'confirm'
                      ? tt('wizard.save.confirmTitle')
                      : undefined
                }
              >
                {manualSaveAction === 'arm'
        ...
        {phase === 'success' && (
          <Stack gap={12} align="center" py={8}>
            ...
            <Stack gap={2} align="center">
              <Text size="md" fw={600} c="var(--nomi-ink)">{tt('wizard.success.added', { name: resultLabel })}</Text>
              <Text size="sm" c="var(--nomi-ink-60)">{tt('wizard.success.body')}</Text>
            </Stack>
            <Group justify="center" gap={8} w="100%" mt={4}>
              <DesignButton variant="subtle" onClick={() => { resetToInput() }}>{tt('wizard.success.addAnother')}</DesignButton>
              <DesignButton variant="filled" onClick={onClose}>{tt('wizard.success.done')}</DesignButton>
            </Group>
          </Stack>
        )}
        ...
        {phase === 'error' && (
          <Stack gap="sm">
            <Text size="md" c="var(--nomi-ink)">{tt('wizard.error.title')}</Text>
            <Text size="sm" c="var(--nomi-ink)">{errorReason}</Text>
            {errorHint && <Text size="sm" c="var(--nomi-ink-60)">{errorHint}</Text>}
            <Group justify="flex-end">
              <DesignButton variant="subtle" onClick={resetToInput}>{tt('wizard.error.retry')}</DesignButton>
              <DesignButton onClick={onClose}>{tt('wizard.error.close')}</DesignButton>
            </Group>
          </Stack>
        )}
      </Stack>
