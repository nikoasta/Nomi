...
const SOURCE_OPTIONS: Source[] = ['mine', 'builtin']
...
export function SkillLibraryContent({
  ...
  const handleNewWithAi = React.useCallback(
    () => gotoCreationWith({ key: 'workbench.creation.skill-author', name: t('skillLibrary.authorName') }),
    [gotoCreationWith, t],
  )
  ...
  const handleExport = React.useCallback(
    (skill: SkillListItemDto) => {
      ...
      if (!pkg) {
        showInfoToast(t('skillLibrary.exportFailed'))
        return
      }
  ...
  const handleDelete = React.useCallback(
    (skill: SkillListItemDto) => {
      ...
      if (!res.ok) {
        showInfoToast(res.error ?? t('skillLibrary.deleteFailed'))
        return
      }
      showUndoToast({
        message: t('skillLibrary.deleted', { name: skill.label }),
        onUndo: () => {
    ...
    (file: File) => {
      ...
      reader.onload = () => {
        ...
        try {
          ...
          showInfoToast(t('skillLibrary.importInvalid'))
          return
        }
        const res = importPackage(parsed)
        showInfoToast(
          res.ok
            ? t('skillLibrary.importSuccess', { name: res.skillName ?? t('skillLibrary.newSkill') })
            : t('skillLibrary.importFailed', { message: res.error ?? t('modelSetup.actionFailed') }),
        )
      }
      reader.onerror = () => showInfoToast(t('skillLibrary.importReadFailed'))
  ...
  const sourceTabs = (
    <div
      ...
      role="tablist"
      aria-label={t('skillLibrary.source.aria')}
    >
      {SOURCE_OPTIONS.map((option) => {
        const activeOption = source === option
        return (
          <button
            key={option}
            type="button"
          ...
          >
            {option === 'mine' ? t('skillLibrary.source.mine') : t('skillLibrary.source.builtin')}
          </button>
        )
      })}
    </div>
  ...
  const newWithAiButton = (
    ...
    >
      ...
      {compact ? t('skillLibrary.newAiCompact') : t('skillLibrary.newAi')}
    </button>
  )
      ...
      <div className={cn('flex min-h-0 flex-1 flex-col overflow-hidden', className)}>
        ...
        {showHeader ? (
          <div className={cn('flex items-center gap-2 px-5 pt-4 pb-3 border-b border-nomi-line')}>
            ...
            <b className={cn('text-title font-bold text-nomi-ink')}>{t('skillLibrary.title')}</b>
            ...
            {onClose ? (
              <button
                ...
                aria-label={t('skillLibrary.close')}
                onClick={onClose}
              >
        ...
        <div className={cn('flex gap-2', compact ? 'flex-col px-3 py-3' : 'items-center px-5 py-2.5')}>
          ...
          <DesignSearchInput
            ...
            ariaLabel={t('skillLibrary.search')}
            value={query}
            onChange={setQuery}
          />
          {compact ? (
        ...
        <div className={cn('flex-1 overflow-y-auto', compact ? 'px-3 pb-3' : 'px-5 pb-5')}>
          {!visible.length && !showNewTile ? (
            <DesignEmptyState
              title={query.trim() ? t('skillLibrary.noMatch.title') : source === 'mine' ? t('skillLibrary.noMine.title') : t('skillLibrary.noBuiltin.title')}
              description={
                query.trim()
                  ? t('skillLibrary.noMatch.description')
                  : source === 'mine'
                    ? t('skillLibrary.noMine.description')
                    : ''
              }
            />
          ) : (
            ...
            >
              {showNewTile ? (
                ...
                >
                  ...
                  <span className={cn('text-caption')}>{t('skillLibrary.newTile')}</span>
                </button>
              ) : null}
              {visible.map((skill) => (
                <SkillCard
  ...
  return (
    <Portal>
      ...
      >
        <div
          role="dialog"
          aria-label={t('skillLibrary.dialog.aria')}
