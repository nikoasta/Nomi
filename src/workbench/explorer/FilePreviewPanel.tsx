...
export function FilePreviewPanel(): JSX.Element | null {
  ...
  return createPortal(
    <div
      ref={panelRef}
      role="dialog"
      aria-label={t('filePreview.aria', { name: node.name })}
    ...
    >
      <div className="flex items-center gap-1 px-3 border-b border-nomi-line shrink-0" style={{ height: 44 }}>
        ...
        <button type="button" onClick={reveal} title={t('filePreview.reveal')} className="p-1 text-nomi-ink-60 hover:text-nomi-ink">
          <IconExternalLink size={16} stroke={1.6} />
        </button>
        <button type="button" onClick={close} aria-label={t('filePreview.close')} className="p-1 text-nomi-ink-60 hover:text-nomi-ink">
    ...
    default:
      return (
        <div className="flex flex-col items-center justify-center gap-1 py-10 text-center">
          <span className="text-body-sm text-nomi-ink-60">{t('filePreview.unsupported')}</span>
          <span className="text-caption text-nomi-ink-40">{t('filePreview.unsupportedHint')}</span>
        </div>
      )
  }
}
...
function TextPreview({ url, markdown }: { url: string; markdown: boolean }): JSX.Element {
  ...
  if (state.loading) return <div className="text-body-sm text-nomi-ink-40">{t('filePreview.loading')}</div>
  if (state.error) return <div className="text-body-sm text-workbench-danger">{t('filePreview.readFailed', { message: state.error })}</div>
