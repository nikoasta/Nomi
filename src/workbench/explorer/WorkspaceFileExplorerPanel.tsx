      ...
      <header className="flex h-12 shrink-0 items-center border-b border-nomi-line-soft px-3">
        ...
        <div className="ml-2 flex shrink-0 items-center gap-1">
          <button
            ...
            aria-label={t('workspaceFiles.listView')}
            aria-pressed="true"
            title={t('workspaceFiles.listView')}
          >
          ...
          <button
            ...
            aria-label={t('workspaceFiles.sort')}
            aria-pressed={!sortAscending}
            title={sortAscending ? t('workspaceFiles.sortAscending') : t('workspaceFiles.sortDescending')}
          >
            {sortAscending ? (
          ...
          <button
            ...
            className={ASSET_PANEL_ICON_BUTTON_CLASS}
            aria-label={t('workspaceFiles.import')}
            title={t('workspaceFiles.import')}
          >
      ...
      {projectId ? (
        <div className="shrink-0 px-3 pt-3">
          <button
            ...
            title={t('workspaceFiles.importTitle')}
          ...
          >
            ...
            <span className="mt-2 text-caption font-semibold leading-tight">
              {importing ? t('workspaceFiles.importing') : t('workspaceFiles.import')}
            </span>
            <span className="mt-1 text-micro leading-tight text-nomi-ink-40">
              {t('workspaceFiles.mediaTypes')}
            </span>
          </button>
        </div>
      ) : null}
      <div className="min-h-0 flex-1 overflow-auto px-2.5 py-3">
        {!projectId ? <p className="px-2 py-4 text-caption text-nomi-ink-40">{t('workspaceFiles.noProject')}</p> : null}
        {loading ? <p className="px-2 py-4 text-caption text-nomi-ink-40">{t('workspaceFiles.loading')}</p> : null}
        ...
        {!loading && !error && projectId && visibleItems.length === 0 ? (
          <DesignEmptyState
            ...
            title={t('workspaceFiles.empty.title')}
            description={t('workspaceFiles.empty.description')}
          />
        ) : null}
        {!loading && !error ? (
        ...
        {truncated ? <p className="px-2 py-2 text-micro text-nomi-ink-40">{t('workspaceFiles.truncated')}</p> : null}
      </div>
    </div>
  )
}
