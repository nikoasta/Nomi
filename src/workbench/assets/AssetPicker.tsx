...
function PickerItem({ asset, onPick, dimmed }: { asset: AssetRef; onPick: (asset: AssetRef) => void; dimmed?: boolean }): JSX.Element {
  ...
  return (
    <button
      ...
      title={dimmed ? t('assetPicker.limitReached') : undefined}
      className={cn(
    ...
    >
      ...
      {canvasAssets.length ? (
        <div className={cn('flex flex-col gap-[6px]')}>
          <span className={cn('text-nomi-ink-40 text-micro')}>{t('assetPicker.canvas')}</span>
      ...
      {projectAssets.length ? (
        <div className={cn('flex flex-col gap-[6px]')}>
          <div className={cn('flex items-baseline justify-between')}>
            <span className={cn('text-nomi-ink-40 text-micro')}>{t('assetPicker.projectRecent')}</span>
            {onBrowseAll ? (
              <button type="button" onClick={onBrowseAll} className={cn('text-nomi-accent text-micro cursor-pointer')}>{t('assetPicker.browseAll')}</button>
            ) : null}
          </div>
      ...
      {loading && !canvasAssets.length && !projectAssets.length ? (
        <div className={cn('grid grid-cols-[repeat(5,48px)] gap-[6px] py-[6px]')} aria-label={t('assetPicker.loading')}>
        ...
        <div className={cn('text-nomi-ink-40 text-micro text-center py-[6px]')}>
          {query ? t('assetPicker.noMatch') : t('assetPicker.empty')}
        </div>
      ) : null}
      ...
      <label className={cn('relative flex items-center justify-center gap-[6px] h-[34px] rounded-nomi-sm border border-dashed border-nomi-ink-20 bg-nomi-paper text-nomi-ink-60 text-xs cursor-pointer hover:border-nomi-accent hover:text-nomi-accent')}>
        ...
        {uploading ? <NomiLoadingMark size={15} label={t('assetPicker.uploadingLabel')} /> : <IconUpload size={15} stroke={2} />}
        {uploading ? t('assetPicker.uploading') : t('assetPicker.uploadLocal')}
        <input
          type="file"
          accept={acceptAttr}
          aria-label={t('assetPicker.uploadLocal')}
          disabled={uploading}
      ...
      <div className={cn('text-nomi-ink-40 text-micro text-center')}>{t('assetPicker.footer')}</div>
    </div>
  )
}
