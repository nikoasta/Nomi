...
export default function TimelinePanel({ density = 'compact', regionLabel, actionLabelPrefix, showTextTrack = false, onCollapse }: TimelinePanelProps): JSX.Element {
  ...
  const handleAiArrange = React.useCallback(() => {
    void import('../generationCanvas/agent/sendStoryboardToTimeline').then(({ arrangeStoryboardToTimeline }) => {
      ...
      if (result.sent.length > 0) toast(t('timeline.aiArrange.success', { count: result.sent.length }), 'success')
      else if (result.total === 0) toast(t('timeline.aiArrange.empty'), 'info')
      else toast(t('timeline.aiArrange.already'), 'info')
    })
  }, [t])
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          {hasSelection ? (
            <div className={cn(
            ...
            )} aria-label={t('timeline.selectedActions')}>
              {primaryMediaClip ? (
                <WorkbenchIconButton
                  ...
                  label={t('timeline.regenerateShot')}
                  title={t('timeline.regenerateShotTitle')}
              ...
              <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.nudgeEarlier')} icon={<IconArrowLeft size={14} />} onClick={() => nudgeTimelineClip(primaryClipId, -1)} />
              <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.copyClip')} icon={<IconCopy size={14} />} onClick={() => duplicateTimelineClip(primaryClipId)} />
              <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.nudgeLater')} icon={<IconArrowRight size={14} />} onClick={() => nudgeTimelineClip(primaryClipId, 1)} />
            </div>
          ) : null}
          {/* C2 一键拼片：把画布镜头按镜序排进时间轴（accent，主操作权重）。 */}
          <WorkbenchIconButton
            className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-accent)] shadow-none cursor-pointer hover:bg-[var(--workbench-accent-soft)]')}
            label={t('timeline.aiArrange')}
            title={t('timeline.aiArrangeTitle')}
          ...
          <WorkbenchIconButton
            ...
            label={splitMode ? t('timeline.splitExit') : t('timeline.splitEnter')}
            title={splitMode ? t('timeline.splitExitTitle') : t('timeline.splitEnterTitle')}
            icon={<IconCut size={14} />}
            onClick={() => setTimelineSplitMode(!splitMode)}
          />
          {canRedo ? (
            <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.redo')} title={t('timeline.redoTitle')} icon={<IconArrowForwardUp size={14} />} onClick={() => redoTimeline()} />
          ) : null}
          {canUndo ? (
            <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.undo')} title={t('timeline.undoTitle')} icon={<IconArrowBackUp size={14} />} onClick={() => undoTimeline()} />
          ) : null}
          <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.zoomOut', { prefix: actionLabelPrefix })} icon={<IconMinus size={14} />} onClick={() => setTimelineZoom(timeline.scale / 1.25)} />
          <span className="text-micro opacity-60 min-w-[32px] text-center">{Math.round(timeline.scale * 100)}%</span>
          <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.zoomReset')} icon={<IconRefresh size={14} />} onClick={() => setTimelineZoom(1)} />
          <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.zoomIn', { prefix: actionLabelPrefix })} icon={<IconPlus size={14} />} onClick={() => setTimelineZoom(timeline.scale * 1.25)} />
          <WorkbenchIconButton className={cn('workbench-timeline__tool', 'w-auto min-w-[30px] h-[var(--workbench-control-size)] px-2 inline-grid place-items-center border-0 rounded-[var(--workbench-control-radius)] bg-transparent text-[var(--workbench-muted)] shadow-none cursor-pointer hover:bg-[var(--workbench-hover)]')} label={t('timeline.deleteSelected', { prefix: actionLabelPrefix })} icon={<IconTrash size={14} />} disabled={!hasSelection} onClick={() => removeSelectedTimelineClips()} />
          {onCollapse ? (
            <WorkbenchIconButton
              ...
              label={t('timeline.collapse', { prefix: actionLabelPrefix })}
