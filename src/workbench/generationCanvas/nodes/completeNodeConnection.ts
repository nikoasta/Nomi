...
export function completeNodeConnection(targetNodeId: string): void {
  ...
  if (!verdict.ok && verdict.reason === 'source_not_referenceable') {
    showInfoToast(canvasRuntimeTranslate('connection.sourceNotReferenceable'))
    return
  }
  if (!verdict.ok && verdict.reason === 'unsupported_reference') {
    showInfoToast(canvasRuntimeTranslate('connection.unsupportedReference'))
    return
  }
  ...
  if (verdict.ok && sourceNodeId) {
    ...
    if (target) {
      ...
      if (hasEdge && !landed) {
        ...
        showInfoToast(maxOfSlots > 0 ? canvasRuntimeTranslate('connection.slotFull', { count: maxOfSlots }) : canvasRuntimeTranslate('connection.referenceFull'))
      }
    }
  }
}
