...
export function Scene3DEnvironmentPanel({
  ...
  const handlePanoramaFile = React.useCallback((file: File) => {
    ...
    if (!file.type.startsWith('image/')) {
      toast(t3d('environment.imageOnly'), 'warning')
      return
    }
    if (file.size > PANORAMA_IMPORT_MAX_BYTES) {
      toast(t3d('environment.fileTooLarge'), 'warning')
      return
    }
    ...
    void (async () => {
      try {
        ...
        try {
          ...
          toast(t3d('environment.dimensionFailed'), 'warning')
          return
        }
        if (panoramaImportRunRef.current !== importRunId) return
        if (!isStandardPanoramaDimensions(dimensions)) {
          toast(t3d('environment.standardRatioRequired', { width: dimensions.width, height: dimensions.height }), 'warning')
          return
        }

        onEnvironmentPatch({
          panoramaUrl: previewUrl,
          panoramaFileName: file.name || t3d('environment.panoramaFallback'),
          showSky: false,
        ...
        toast(t3d('environment.imported'), 'success')
      } catch {
        try {
  ...
  return (
    <div className="grid gap-3">
      <div className="flex items-center justify-between gap-2 text-caption text-[var(--nomi-ink-60)]">
        <label htmlFor="scene3d-dark-mode">{t3d('environment.darkMode')}</label>
        <Switch
      ...
      <div className="flex items-center justify-between gap-2 text-caption text-[var(--nomi-ink-60)]">
        <label htmlFor="scene3d-show-grid">{t3d('environment.showGrid')}</label>
        <Switch
      ...
      <div className="flex items-center justify-between gap-2 text-caption text-[var(--nomi-ink-60)]">
        <label htmlFor="scene3d-show-axes">{t3d('environment.showAxes')}</label>
        <Switch
      ...
      <div className="flex items-center justify-between gap-2 text-caption text-[var(--nomi-ink-60)]">
        <label htmlFor="scene3d-show-sky">{t3d('environment.showSky')}</label>
        <Switch
          id="scene3d-show-sky"
      ...
      <EnvironmentColorField
        label={t3d('environment.backgroundColor')}
      ...
      <div className="grid gap-2 rounded-nomi border border-[var(--nomi-line-soft)] bg-[var(--nomi-ink-05)] p-2">
        <div className="flex items-center justify-between gap-2">
          <span className="inline-flex min-w-0 items-center gap-1.5 text-caption font-medium text-[var(--nomi-ink)]">
            ...
            <span className="truncate">{t3d('environment.panorama')}</span>
          </span>
          {environment.panoramaUrl ? (
            <button
              ...
              disabled={readOnly}
              type="button"
              title={t3d('environment.removePanorama')}
        ...
        {environment.panoramaUrl ? (
          <div className="grid gap-2">
            ...
            <div className="min-w-0 truncate text-micro text-[var(--nomi-ink-60)]">
              {environment.panoramaFileName || t3d('environment.panoramaFallback')}
            </div>
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('environment.rotation')}</span>
            ...
            <div className="grid gap-2 rounded-nomi-sm border border-[var(--nomi-line-soft)] bg-[var(--nomi-paper)] p-2">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('environment.displayMode')}</span>
              <div className="grid grid-cols-2 gap-1">
                {([
                  ['panorama', t3d('environment.modePanorama')],
                  ['sphere', t3d('environment.modeSphere')],
            ...
            {environment.environmentMode === 'sphere' ? (
              <div className="grid gap-2 rounded-nomi-sm border border-[var(--nomi-line-soft)] bg-[var(--nomi-paper)] p-2">
                <span className="text-micro font-medium text-[var(--nomi-ink)]">{t3d('environment.sphereRadius')}</span>
                <label className="grid gap-1">
                  <div className="flex items-center justify-between">
                    <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('environment.radius')}</span>
          ...
          >
            <IconUpload size={14} />
            {t3d('environment.importPanorama')}
          </button>
        )}
        <input
