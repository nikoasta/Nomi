...
export default function Scene3DFullscreen({
  ...
  const editorShell = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t3d('fullscreen.aria')}
      tabIndex={0}
    ...
    >
      ...
      <main className="relative flex min-h-0 flex-1 overflow-hidden bg-[var(--workbench-bg)]">
        ...
        <div className="relative min-h-0 min-w-0 flex-1 overflow-hidden bg-[var(--nomi-ink-05)]">
          ...
          {!leftPanelOpen ? (
            <CanvasPanelRestoreButton side="left" title={t3d('fullscreen.showSceneNodes')} onClick={() => setLeftPanelOpen(true)}>
              <IconListTree size={18} />
            </CanvasPanelRestoreButton>
          ) : null}
          {!rightPanelOpen ? (
            <CanvasPanelRestoreButton side="right" title={t3d('fullscreen.showProperties')} onClick={() => setRightPanelOpen(true)}>
