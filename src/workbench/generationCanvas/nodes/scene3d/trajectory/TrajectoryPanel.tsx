    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          type="button"
          title={t3d('trajectory.deleteControlPoint')}
  ...
  return (
    <div className={cn(
      ...
      <button className="flex min-w-0 items-center gap-2 text-left" type="button" onClick={() => onSelectTrajectory(trajectory.id)}>
        ...
        <span className="shrink-0 text-micro text-[var(--nomi-ink-40)]">{t3d('trajectory.nodesCount', { count: binding?.objects.length ?? 0 })}</span>
      </button>
        ...
        <>
          <div className="grid grid-cols-2 gap-2">
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.start')}</span>
              <input
            ...
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.end')}</span>
              <input
                className="h-8 min-w-0 rounded-nomi-sm border border-[var(--nomi-line)] bg-[var(--nomi-paper)] px-2 text-caption text-[var(--nomi-ink)] outline-none"
                disabled={readOnly}
                min={0.1}
          ...
          <label className="grid gap-1">
            <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.direction')}</span>
            <NomiSelect
              ariaLabel={t3d('trajectory.direction')}
              disabled={readOnly}
          ...
          {binding.objects.map((boundObject) => (
            <div key={boundObject.objectId} className="grid grid-cols-[minmax(0,1fr)_72px_28px] items-end gap-1">
              ...
              <button
                ...
                disabled={readOnly}
                type="button"
                title={t3d('trajectory.unbindNode')}
          ...
          <NomiSelect
            ariaLabel={t3d('trajectory.addBindingNode')}
            placeholder={t3d('trajectory.addBindingNode')}
          ...
          >
            {t3d('trajectory.deleteBinding')}
          </button>
        </>
      ) : (
        <NomiSelect
          ariaLabel={t3d('trajectory.selectNodeCreateBinding')}
          placeholder={t3d('trajectory.selectNodeCreateBinding')}
          disabled={readOnly || availableNodes.length === 0}
          value=""
          options={availableNodes.map((node) => ({ value: node.id, label: node.name }))}
          onChange={(objectId) => {
  ...
  return (
    <section className="min-h-0 flex-1 overflow-auto bg-[var(--nomi-paper)] px-3 py-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="text-caption font-medium text-[var(--nomi-ink)]">{t3d('trajectory.properties')}</div>
        <button
      ...
      {!activeTrajectory ? (
        ...
        <div className="grid gap-3">
          <div className="flex min-w-0 items-center gap-2 rounded-nomi-sm bg-[var(--nomi-ink-05)] px-2 py-2">
            ...
            <span className="text-micro text-[var(--nomi-ink-40)]">{t3d('trajectory.pointNodeCount', { points: activeTrajectory.points.length, nodes: activeBinding?.objects.length ?? 0 })}</span>
          </div>

          <label className="grid gap-1">
            <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.name')}</span>
            <input
          ...
          <div className="grid grid-cols-[32px_minmax(0,1fr)] items-end gap-2">
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.color')}</span>
              <input
            ...
            <label className="grid gap-1">
              <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('trajectory.tension')}</span>
              <input
          ...
          <label className="flex items-center justify-between gap-2 rounded-nomi-sm border border-[var(--nomi-line-soft)] bg-[var(--nomi-ink-05)] px-2 py-2 text-caption text-[var(--nomi-ink-60)]">
            <span>{t3d('trajectory.closed')}</span>
            <input
          ...
          <div className="grid gap-2">
            <div className="text-caption font-medium text-[var(--nomi-ink)]">{t3d('trajectory.currentBinding')}</div>
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between gap-2">
              <div className="text-caption font-medium text-[var(--nomi-ink)]">{t3d('trajectory.controlPoints')}</div>
              <button
              ...
              >
                <IconPlus size={13} />
                <span>{t3d('trajectory.addPoint')}</span>
              </button>
            </div>
          ...
          >
            {t3d('trajectory.deleteTrajectory')}
          </button>
        </div>
      )}
    </section>
