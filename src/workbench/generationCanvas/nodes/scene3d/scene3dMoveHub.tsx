...
export function Scene3DMoveHub({
  ...
  const displayLabel = useScene3DLabel()
  ...
  const takeTargets = [
    ...mannequins.map((object) => ({ kind: 'mannequin' as const, id: object.id, label: t3d('moveHub.mannequinTarget', { name: displayLabel(object.name) }) })),
    ...state.cameras.map((camera) => ({ kind: 'camera' as const, id: camera.id, label: t3d('moveHub.cameraTarget', { name: displayLabel(camera.name) }) })),
  ]
  ...
  return (
    <div className="flex min-h-0 shrink-0 flex-col gap-2 border-t border-[var(--workbench-border)] p-3" data-coach="camera-move-panel">
      <div className="flex items-center gap-2">
        <span className="text-caption font-medium text-[var(--nomi-ink)]">{t3d('moveHub.title')}</span>
        {moveReady ? (
          <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('moveHub.ready')}</span>
        ) : (
          <span className="text-micro text-[var(--nomi-ink-40)]">{t3d('moveHub.chooseMethod')}</span>
        )}
      </div>
      <div className="flex items-center gap-1 rounded-nomi border border-[var(--nomi-line-soft)] bg-[var(--nomi-paper)] p-0.5">
        <PanelButton title={t3d('moveHub.presetTitle')} active={tab === 'preset'} onClick={() => onTabChange('preset')}>
          <IconVideo size={14} />
          <span>{t3d('moveHub.preset')}</span>
        </PanelButton>
        <PanelButton title={t3d('moveHub.trajectoryTitle')} active={tab === 'trajectory'} onClick={() => onTabChange('trajectory')}>
          <IconRoute size={14} />
          <span>{t3d('moveHub.trajectory')}</span>
        </PanelButton>
        <PanelButton title={t3d('moveHub.takeTitle')} active={tab === 'take'} onClick={() => onTabChange('take')}>
          <IconPlayerRecord size={14} />
          <span>{t3d('moveHub.take')}</span>
        </PanelButton>
      </div>
      <div className="min-h-0 overflow-y-auto">
        {tab === 'preset' ? (
          selectedCamera ? (
            ...
            <div className="grid gap-2 rounded-nomi border border-dashed border-[var(--nomi-line-soft)] p-3 text-caption text-[var(--nomi-ink-60)]">
              <span>{t3d('moveHub.selectCameraHint')}</span>
              {state.cameras[0] && !readOnly ? (
                ...
                >
                  {t3d('moveHub.selectCamera', { name: displayLabel(state.cameras[0].name) })}
                </button>
              ) : null}
            </div>
          )
          ...
          <div className="grid gap-2">
            {!readOnly ? (
              <button
                type="button"
                className="h-8 rounded-nomi-sm border border-[var(--nomi-line-soft)] bg-[var(--nomi-ink-05)] px-2 text-caption text-[var(--nomi-ink)] hover:bg-[var(--nomi-ink-10)]"
                title={t3d('moveHub.enterViewportTitle')}
                onClick={onEnterTrajectoryMode}
              >
                {t3d('moveHub.enterViewport')}
              </button>
            ) : null}
            <TrajectoryPanel
              state={state}
        ...
        {tab === 'take' ? (
          <div className="grid gap-2 rounded-nomi border border-[var(--nomi-line-soft)] bg-[var(--nomi-paper)] p-2 text-caption text-[var(--nomi-ink-60)]">
            <span>{t3d('moveHub.takeDescription')}</span>
            {canRecordTake && !readOnly ? (
              <>
                <label className="grid gap-1">
                  <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('moveHub.controlTarget')}</span>
                  <select
                ...
                <button
                  ...
                  title={t3d('moveHub.enterControlTitle')}
                  onClick={() => {
                ...
                >
                  ...
                  {t3d('moveHub.enterControl')}
                </button>
              </>
            ) : (
              <span className="text-micro text-[var(--nomi-ink-40)]">{t3d('moveHub.takeUnsupported')}</span>
            )}
          </div>
        ) : null}
      </div>
