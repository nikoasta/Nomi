...
export function CameraMovePanel({
  ...
  return (
    <div className="grid gap-2 rounded-nomi border border-[var(--nomi-line-soft)] bg-[var(--nomi-paper)] p-2">
      <div className="text-caption font-medium text-[var(--nomi-ink)]">{t3d('cameraMove.title')}</div>
      <div
      ...
      >
        <div className="flex min-w-0 items-center gap-1.5 text-caption font-medium text-[var(--nomi-ink)]">
          ...
          <span>{t3d('cameraMove.referenceOutput')}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="grid gap-1">
          <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('cameraMove.duration')}</span>
          <input
        ...
        <label className="grid gap-1">
          <span className="text-micro text-[var(--nomi-ink-60)]">{t3d('cameraMove.amplitude')}</span>
          <input
            className="h-8 min-w-0 rounded-nomi-sm border border-[var(--nomi-line)] bg-[var(--nomi-paper)] px-2 text-caption text-[var(--nomi-ink)] outline-none focus:border-[var(--nomi-accent)] disabled:opacity-50"
            disabled={readOnly}
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {CAMERA_MOVES.map((move) => (
          <button
            ...
            disabled={readOnly}
            type="button"
            title={move === 'dolly_zoom' ? t3d('cameraMove.dollyZoomHint') : undefined}
            onClick={() => applyMove(move)}
          >
            {t3d(`cameraMove.${move}` as Scene3DI18nKey)}
          </button>
        ))}
      </div>
      <button
        ...
        disabled={readOnly}
        type="button"
        title={t3d('cameraMove.exportFramesTitle')}
        onClick={onExportFrames}
      >
        {t3d('cameraMove.exportFrames')}
      </button>
