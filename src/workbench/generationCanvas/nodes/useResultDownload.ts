...
export function useResultDownload(node: GenerationCanvasNode): {
  ...
  const download = React.useCallback(() => {
    ...
    if (!bridge) return
    const base = (node.title || '').trim() || (type === 'video' ? t('resultDownload.defaultVideoName') : t('resultDownload.defaultImageName'))
    const urlExt = /\.[a-z0-9]{1,5}(?:$|\?)/i.test(url) ? '' : type === 'video' ? '.mp4' : '.png'
    setDownloading(true)
    void bridge.assets
      .download({ url, suggestedName: base + urlExt })
      .then((res) => {
        if (res.ok) toast(t('resultDownload.success'), 'success')
        else if (!res.canceled) toast(t('resultDownload.failed'), 'error')
      })
      .catch((error: unknown) => toast(error instanceof Error ? error.message : t('resultDownload.failed'), 'error'))
