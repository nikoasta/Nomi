...
export function NodeImageLightbox({ open, src, title, onClose }: Props): JSX.Element | null {
  ...
  const label = (title || '').trim() || t('mediaPreview.imageResult')
  ...
  return (
    <BodyPortal>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t('mediaPreview.imageDialog', { title: label })}
      ...
      >
        <div className="pointer-events-none absolute left-6 top-5 right-20 flex min-w-0 items-center gap-2 text-nomi-paper">
          ...
          <span className="shrink-0 text-caption text-nomi-paper/60">{t('mediaPreview.originalPreview')}</span>
        </div>
        <button
          ref={closeButtonRef}
          type="button"
          ...
          aria-label={t('mediaPreview.closeLightbox')}
          title={t('mediaPreview.closeEsc')}
          onClick={onClose}
        >
        ...
        <NomiImage
          ...
          eager
          alt={label}
          fallbackLabel={t('mediaPreview.originalLoadFailed')}
          fallbackTitle={t('mediaPreview.originalStale')}
          className={cn(
        ...
        />
        <span className="pointer-events-none absolute bottom-5 text-caption text-nomi-paper/60">{t('mediaPreview.backgroundCloseHint')}</span>
      </div>
    </BodyPortal>
  )
}
