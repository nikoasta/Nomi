...
import { getHandbookContent } from './handbookContent'
...
export function HandbookPanel({ opened, onClose }: Props): JSX.Element | null {
  ...
  const handbook = React.useMemo(() => getHandbookContent(locale), [locale])
  ...
  return (
    <Portal>
      ...
      >
        <div
          role="dialog"
          aria-label={t('handbook.dialog.aria')}
        ...
        >
          ...
          <div className={cn('flex items-center gap-2 px-5 pt-4 pb-3 border-b border-nomi-line')}>
            ...
            <b className={cn('text-title font-bold text-nomi-ink')}>{handbook.title}</b>
          ...
          <div className={cn('flex-1 overflow-auto px-5 py-4 flex flex-col gap-5')}>
            <p className={cn('text-caption text-nomi-ink-40 m-0')}>{handbook.subtitle}</p>

            {/* 流水线一行图 */}
            <div className={cn('rounded-nomi bg-nomi-ink-05 px-4 py-3')}>
              <div className={cn('text-micro text-nomi-ink-40 mb-2')}>{handbook.pipelineIntro}</div>
              <div className={cn('flex items-center flex-wrap gap-1.5')}>
                {handbook.pipeline.map((step, i) => (
                  <React.Fragment key={step.label}>
                    ...
                    {i < handbook.pipeline.length - 1 ? <IconArrowRight size={14} stroke={1.6} className={cn('text-nomi-ink-40')} /> : null}
                  </React.Fragment>
                ))}
              </div>
            </div>
            ...
            <section className={cn('flex flex-col gap-2')}>
              <SectionTitle>{handbook.firstWinTitle}</SectionTitle>
              <p className={cn('text-caption text-nomi-ink-60 m-0')}>{handbook.firstWinBody}</p>
              <div className={cn('grid gap-2.5 grid-cols-2 max-[560px]:grid-cols-1')}>
                {handbook.firstWin.map((step) => (
            ...
            {/* 我想做 X → 走这条路 */}
            <section className={cn('flex flex-col gap-2')}>
              <SectionTitle>{handbook.routesTitle}</SectionTitle>
              <p className={cn('text-caption text-nomi-ink-60 m-0')}>{handbook.routesBody}</p>
              <div className={cn('flex flex-col border border-nomi-line rounded-nomi overflow-hidden')}>
                {handbook.intentRoutes.map((route, i) => (
                  <div
                    key={route.title}
                    className={cn(
            ...
            {/* 卡住了看这里 */}
            <section className={cn('flex flex-col gap-2')}>
              <SectionTitle>{handbook.gotchasTitle}</SectionTitle>
              <div className={cn('grid gap-2.5 grid-cols-2 max-[560px]:grid-cols-1')}>
                {handbook.gotchas.map((g) => (
